...
```text
Terminal -> Ruta -> Flota (una o varias) -> VehiculoTransporte
```

Una ruta nueva no tiene flotas automáticamente. Las flotas o asociaciones se agregan
por separado a una ruta existente, cada una con un nombre, sus turnos, vehículos y
tarifas propias. La opción 2 del menú crea una nueva flota y solicita la ruta a la que
se asociará. Los archivos JSON antiguos sin nombre de asociación se migran como
`Asociación migrada`.
...
- Cada flota o asociación mantiene sus propias tarifas para `estandar`, `ejecutivo` y `taxi`.
- Cada flota ofrece una tarifa base y un recargo por tipo; la tarifa final es la suma de ambos valores y puede ser distinta entre asociaciones de la misma ruta.
...
Para abordar, el pasajero selecciona la ruta y la flota o asociación, luego el tipo de servicio (`estandar`, `ejecutivo` o `taxi`) y confirma con `SI`. Luego indica si es estudiante; si no lo es, indica si tiene 60 años o más. Si cumple cualquiera de las dos condiciones no se aplica el recargo de la flota; de lo contrario, se aplica. La flota asigna automáticamente el primer vehículo registrado de ese tipo que tenga capacidad suficiente; cuando se llena, continúa con el siguiente. El identificador o placa no se solicita al pasajero: aparece en la factura de abordaje junto con la ruta, la flota, el servicio, la cantidad, el recargo aplicado y el total.

Si la flota no tiene ningún vehículo disponible del tipo seleccionado, el abordaje se cancela inmediatamente, no se solicita la información restante ni se genera factura, y el sistema regresa al menú principal.
...
El menú permite eliminar rutas, flotas y vehículos solicitando una confirmación `SI`; la opción 13 elimina una flota de la ruta seleccionada y los datos se guardan inmediatamente después de eliminar.

También valida cada dato antes de registrarlo. Las opciones de configuración y modificación solicitan primero la ruta y la flota, y luego tarifa base y recargo para `estandar`, `ejecutivo` y `taxi`; cada asociación guarda sus propios precios. Si se introduce un tipo incorrecto, un texto vacío, un número inválido, una opción inexistente o una cantidad fuera de rango, el sistema muestra un mensaje y vuelve a solicitar ese dato hasta recibir uno válido. Los espacios al principio y al final se eliminan automáticamente; por ejemplo, `  Caracas  ` se guarda como `Caracas`.

Las tarifas de cada flota se configuran una sola vez mediante la opción 4. Si una flota ya tiene sus tres tarifas definidas, esa opción no las sobrescribe. Los cambios posteriores solo se pueden realizar mediante la opción 5, `Modificar tarifas y recargos de la flota`.
